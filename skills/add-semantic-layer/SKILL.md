...
dbt MetricFlow (Cloud Semantic Layer mode) is a metric store. Snowflake views and `nao semantic files` are semantic layers. The reliability gain is bigger with a metric store; the scope reduction is also bigger.
...
| **Other**                            | Varies         | Anything else (Looker / LookML, Cube, AtScale, etc.).                   |
...
- `templates/semantic.yaml` — single-file schema for the nao semantic layer (Path D). Holds all dimensions and metrics for the project.
